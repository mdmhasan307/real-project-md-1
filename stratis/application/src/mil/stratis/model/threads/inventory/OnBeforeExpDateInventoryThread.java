...
@Slf4j
public class OnBeforeExpDateInventoryThread extends BaseInventoryThread {

  public OnBeforeExpDateInventoryThread(DBTransaction db, String dbConnectionForUser, AdfDbCtxLookupUtils adfDbCtxLookupUtils) {
  ...
  public void init(Object expirationDate, int iUserId, Object InventoryID) {
  ...
  protected void doRun() throws SQLException {
    ...
    try {
      ...
      log.info("running... ExpirationDate: {} InventoryId:  {}", expirationDate.toString(), inventoryIdStr);
      setStatus("RUNNING");
      String sql = "select niin_id, niin_loc_id, wac_id, qty, l.location_id " +
          ...
          "and n.qty > 0 and n.expiration_date <= ? " +
      ...
      String sqlINS = "insert into inventory_item (niin_id, niin_loc_id, inventory_id, num_counts, created_by, created_date, modified_date, inv_type, wac_id, priority, status, niin_loc_qty, location_id) values (?,?,?,0,?,sysdate,sysdate,'INVENTORY',?,1,'INVENTORY',?,?)";
      pstmtINS = super.createPreparedStatement(sqlINS, 0);

      pstmt.setObject(1, expirationDate);
