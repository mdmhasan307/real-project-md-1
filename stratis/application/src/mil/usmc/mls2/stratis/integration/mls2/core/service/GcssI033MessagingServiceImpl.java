...
public class GcssI033MessagingServiceImpl implements GcssI033MessagingService {

  private static final String GABF_INTERFACE_NAME = "GABF";
  ...
  public void processMessage(InboundMessaging inboundMessage) {
    ...
    try {
      val gabfDataResponse = CmdPayloadUtils.toMls2Message(inboundMessage, GabfDataResponse.class, true);
      InternalSiteSelectionTracker.configureTracker(gabfDataResponse.getSiteIdentifier());
      //      val requestDasfFeedId = dasfDataResponse.getDasfFeedId(); // do we need this?

      // We want to push this XML into GCSSMC_IMPORTS_DATA
      val feedXml = gabfDataResponse.getFeedXml();
      ...
      val gcssMcImportsData = GcssMcImportsData.builder()
          .interfaceName(GABF_INTERFACE_NAME)
          .createdBy(1)
      ...
      val correlationId = gabfDataResponse.getMeta().getCorrelationId();
