...
public class GcssI112MatsMessagingServiceImpl implements GcssI112MatsMessagingService {

  private static final String MATS_INTERFACE_NAME = "MATS";
  ...
  public void processMessage(InboundMessaging inboundMessage) {
    ...
    try {
      val matsDataResponse = CmdPayloadUtils.toMls2Message(inboundMessage, MatsDataResponse.class, true);
      InternalSiteSelectionTracker.configureTracker(matsDataResponse.getSiteIdentifier());

      // We want to push this XML into GCSSMC_IMPORTS_DATA
      val feedXml = matsDataResponse.getFeedXml();
      ...
      val gcssMcImportsData = GcssMcImportsData.builder()
          .interfaceName(MATS_INTERFACE_NAME)
          .createdBy(1)
      ...
      val correlationId = matsDataResponse.getMeta().getCorrelationId();
