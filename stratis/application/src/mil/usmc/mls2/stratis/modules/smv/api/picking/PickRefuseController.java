...
@Slf4j
@RestController
@RequestMapping(value = MappingConstants.PICKING_REFUSE)
@RequiredArgsConstructor
@SuppressWarnings("Duplicates")
public class PickRefuseController {
  ...
  private static final String DEFAULT_PAGE = "mobile/picking/refuse";
  ...
  public SpaGetResponse show(HttpServletRequest request) {
    try {
      ...
      return SMVUtility.processGetResponse(request, DEFAULT_PAGE, "Refuse Detail");
    }
    catch (Exception e) {
      log.error("Error occurred processing Pick Refuse", e);
  ...
  public SpaPostResponse post(PickingDetailInput pickingDetailInput, HttpServletRequest request) {
    try {
      ...
      pickingProcessor.processPickRefuse(pick, pickingDetailInput, userId);
      ...
      log.error("Error occurred processing Pick Refuse post", e);
